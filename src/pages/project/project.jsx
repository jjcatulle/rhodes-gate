...
import { ProjectPageCtn } from "./styles";
import { TeamCtn, TeamItem } from "../../components/Partners/Partners.styles";
...
function ActionAreaCard({
  ...
  return (
    <Card className="card" style={{ borderRadius: "2rem" }}>
      <CardActionArea>
        <CardMedia
          component="img"
          // height="500"
...
function BadgeAvatars({ name, role, image }) {
  return (
    <TeamItem>
      <Badge
        overlap="circular"
...
const ProjectPage = () => {
  let { id } = useParams();
  let currentItem = myData.find((i) => i.id == id);

  return (
    <ProjectPageCtn image={currentItem.image}>
      <div className="left">
        <div className="ctn">
          <ActionAreaCard image={currentItem.image} />
        </div>
      </div>
      <div className="right">
        <div className="ctn">
          <div className="title">{currentItem.title}</div>
          <div className="desc">{currentItem.description}</div>
          <div className="date">
            Release Date: <span>{currentItem.date}</span>
          </div>
